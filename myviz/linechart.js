...
const multilineMargin = {
  ...
  const multilineSvg = d3
    ...
    .attr(
      'width',
      multilineWidth +
        multilineMargin.left +
        multilineMargin.right -
        50,
    )
    .attr(
      'height',
      multilineHeight +
        multilineMargin.top +
        multilineMargin.bottom -
        50,
    )
    .append('g')
    .attr(
      'transform',
      `translate(${multilineMargin.left},${multilineMargin.top})`,
    );
  ...
  d3.csv("./finalprojdata/linechartdata.csv").then((data) => {
    ...
    multilineSvg
      ...
      .attr(
        'cx',
        (d) =>
          multilineX(d.Year) + multilineX.bandwidth() / 2,
      )
